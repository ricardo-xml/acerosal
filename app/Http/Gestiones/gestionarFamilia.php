...
<table border="1" cellpadding="5">
  ...
  <tbody>
    <?php while($r = $rows->fetch_assoc()): ?>
      <tr>
        <form method="POST" action="<?= BASE_URL ?>controladores/ControladorFamilia.php?accion=actualizar">
          <td>
            <input type="hidden" name="idFamilia" value="<?= $r['idFamilia'] ?>">
          ...
          <td>
            ...
            <a href="<?= BASE_URL ?>controladores/ControladorFamilia.php?accion=eliminar&id=<?= $r['idFamilia'] ?>"
               onclick="return confirm('¿Eliminar esta familia (borrado lógico)?')">🗑️ Eliminar</a>
          </td>
        </form>
      </tr>
