...
<table border="1" cellpadding="5">
  ...
  <tbody>
    <?php while($r = $rows->fetch_assoc()): ?>
      <?php
        // Buscar nombre de padre si lo tiene (consulta ligera, opcional optimizar)
        $padre = '';
        if (!empty($r['id_ModuloPadre'])) {
          $idp = (int)$r['id_ModuloPadre'];
          $q = $cn->query("SELECT Nombre FROM Modulos WHERE idModulos = $idp");
          $padre = ($q && $q->num_rows) ? $q->fetch_assoc()['Nombre'] : '';
        }
      ?>
      <tr>
